...
mdot_box = 0.5
T_box = 4 + 273.15
h_air_box = CPPSI('H', 'T', T_box, 'P', 1e5, air)
p_SL = 1e5
superheat = 4.0
...
srcAir = vcs.Source(id='srcAir', system=system, mdot=mdot_box, p=1e5, h=h_air_box)
...
# junctions glycol loop
hex_evap = vcs.Junction(id='srcSL_evap', system=system, medium=sl, upstream_component=hex, upstream_id='outlet_A', downstream_component=evap, downstream_id='inlet_B', mdot_init=mdot_SL, p_init=p_SL, h_init=h_in_sl)
evap_hex = vcs.Junction(id='evap_snkSL', system=system, medium=sl, upstream_component=evap, upstream_id='outlet_B', downstream_component=hex, downstream_id='inlet_A', mdot_init=mdot_SL, p_init=p_SL, h_init=h_out_SL_init)
srcAir_hex = vcs.Junction(id='srcAir_hex', system=system, medium=air, upstream_component=srcAir, upstream_id='outlet_A', downstream_component=hex, downstream_id='inlet_B', mdot_init=mdot_box, p_init=1e5, h_init=h_air_box)
hex_snkAir = vcs.Junction(id='hex_snkAir', system=system, medium=air, upstream_component=hex, upstream_id='outlet_B', downstream_component=snkAir, downstream_id='inlet_A', mdot_init=mdot_box, p_init=1e5, h_init=h_air_box)
